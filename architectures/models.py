...
def final_model(input_shape: dict):
    ...
    # dense_video = Dense(128, activation="relu")(encoded_sequence)

    sequences, _, lstm_output = LSTM(256, return_sequences=True, return_state=True,
    ...
    dense_video = Dense(128, activation="relu")(attention_result)
...
def final_model2(input_shape: dict):
    ...
    # encoded_sequence = LSTM(512, recurrent_dropout=0.2, dropout=0.5)(encoded_frames)
    # dense_video = Dense(256, activation="relu")(encoded_sequence)

    sequences, _, lstm_output = LSTM(1024, return_sequences=True, return_state=True,
                                     recurrent_initializer='glorot_uniform')(encoded_frames)
    attention_layer = BahdanauAttention(10)
    attention_result, attention_weights = attention_layer(lstm_output, sequences)
    ...
    concat = Concatenate()([dense_video, audio_global_avg, motion_dense1])
    dp1 = Dropout(0.2)(concat)
    dense1 = Dense(64, activation='relu')(dp1)
    output = Dense(classes, activation='softmax')(dense1)
...
if __name__ == '__main__':
    ...
    model.compile('sgd', 'categorical_crossentropy', metrics=['accuracy'])
    model.summary()
    tf.keras.utils.plot_model(model, to_file='model2.png', show_shapes=True, expand_nested=False)
